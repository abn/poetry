...
from poetry.sources.cached import CachedPackageSource
...
class HTTPPackageSource(CachedPackageSource, ABC):
    def __init__(
        self,
        name: str,
        url: str,
    ...
    def authenticated_url(self) -> str:
        ...
        return f"{parsed.scheme}://{quote(self._session.auth.username, safe='')}:{quote(self._session.auth.password, safe='')}@{parsed.netloc}{parsed.path}"
    ...
    def _get_info_from_wheel(self, url: str) -> "PackageInfo":
        ...
        self._log(
            f"Downloading wheel: {urllib.parse.urlparse(url).path.rsplit('/')[-1]}",
            level="debug",
        )
    ...
    def _get_info_from_sdist(self, url: str) -> "PackageInfo":
        ...
        self._log(
            f"Downloading sdist: {urllib.parse.urlparse(url).path.rsplit('/')[-1]}",
            level="debug",
        )
    ...
    def _links_to_data(self, links: List["Link"], data: "PackageInfo"):
        if not links:
            raise PackageNotFound(
                f'No valid distribution links found for package: "{data.name}" version: "{data.version}"'
            )
    ...
    def _get_response(self, endpoint: str) -> Optional[requests.Response]:
    ...
    def _get(self, endpoint: str) -> Optional[HTMLPageLinkPackageSource]:
        response = self._get_response(endpoint)
        if not response:
            return
        return HTMLPageLinkPackageSource(response.url, response.text)
